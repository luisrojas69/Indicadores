...
/* Ajuste Flex para incluir el buscador y el botón de cámara */
.cat-search-wrap { 
    flex: 1; 
    display: flex; 
    align-items: center; 
...
.btn-camara:active { transform: scale(.95); background: #1e293b; }
...
/* ══ MODAL FICHA Y MODAL CAMARA ══════════════════════════════ */
#modalFicha .modal-content, #modalCamara .modal-content {
...
<div class="cat-shell">
    ...
    <div class="cat-bar">
        <div class="cat-search-wrap">
            ...
            <button class="btn-camara" title="Escanear Código de Barras / QR" data-bs-toggle="modal" data-bs-target="#modalCamara">
...
<div class="modal fade" id="modalCamara" tabindex="-1">
    <div class="modal-dialog modal-md modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header" style="border:none;padding:16px 20px 4px;">
                <h5 class="modal-title fw-bold" style="font-size:16px;color:#0f172a;">
                    <i class="fas fa-qrcode me-2"></i> Escanear Código
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body" style="padding:0 20px 20px;">
                {{-- Contenedor donde la librería de JS pintará la cámara --}}
                <div id="lectorCamara" style="width: 100%; border-radius: 12px; overflow: hidden; background: #000; min-height: 250px;"></div>
                
                <div id="feedbackCamara" class="text-center mt-3 text-muted" style="font-size: 13px;">
                    Presione "Permitir" si el navegador solicita acceso a la cámara.
                </div>
            </div>
        </div>
    </div>
...
/* ═══════════════════════════════════════════════
   PUENTE PARA LA CÁMARA (FASE 2.4)
═══════════════════════════════════════════════ */
window.inyectarCodigoDesdeCamara = (codigo) => {
    // 1. Ocultar el modal de cámara de Bootstrap
    const modalEl = document.getElementById('modalCamara');
    const modalInstance = bootstrap.Modal.getInstance(modalEl);
    if (modalInstance) {
        modalInstance.hide();
    }

    // 2. Inyectar el código en tu buscador actual y disparar la búsqueda
    const inputBuscar = document.getElementById('searchInput');
    if (inputBuscar) {
        inputBuscar.value = codigo;
        // Inyectar el resultado directamente en el flujo del buscador actual
        //executeSearch(); //comentado para evitar recarga inmediata, se puede activar según necesidad
        showFicha(codigo); // Llama directamente al endpoint AJAX que usa el nuevo servicio
    }
};
...
$( document ).ready(function() {
    console.log( "camara ready!" );
    /* ═══════════════════════════════════════════════
    LÓGICA DEL ESCÁNER HTML5-QRCODE (FASE 2.4)
    ═══════════════════════════════════════════════ */
    let html5QrCode = null;
    let isProcessingScan = false; // Flag para evitar lecturas duplicadas (Debouncing)

    // Evento: Cuando el modal de la cámara termina de abrirse
    $('#modalCamara').on('shown.bs.modal', function () {
        isProcessingScan = false;
        const $feedback = $('#feedbackCamara');
        $feedback.html('<i class="fas fa-spinner fa-spin"></i> Solicitando acceso a la cámara...').attr('class', 'text-center mt-3 text-muted');

        // Inicializamos la instancia apuntando al ID del div contenedor
        html5QrCode = new Html5Qrcode("lectorCamara");

        // Configuramos el lector
        const config = { 
            fps: 10, // Escanea 10 veces por segundo (equilibrio perfecto entre rapidez y batería)
            qrbox: { width: 250, height: 150 }, // Rectángulo estilo código de barras
            aspectRatio: 1.0 
        };

        // Iniciamos forzando la cámara trasera ("environment")
        html5QrCode.start(
            { facingMode: "environment" },
            config,
            // Callback de Éxito
            (decodedText, decodedResult) => {
                // Evitar que lea el mismo código 5 veces antes de que se cierre el modal
                if (isProcessingScan) return; 
                isProcessingScan = true;

                $feedback.html('<i class="fas fa-check-circle"></i> ¡Código capturado!').attr('class', 'text-center mt-3 text-success font-weight-bold');

                // Detener la cámara ANTES de procesar (Ahorra memoria y libera hardware en Android)
                html5QrCode.stop().then(() => {
                    html5QrCode.clear();
                    // Mandamos el código al puente que creamos en el paso 2.3
                    window.inyectarCodigoDesdeCamara(decodedText);
                }).catch((err) => {
                    console.error("Fallo al detener cámara:", err);
                    // Aún si falla el stop, procesamos el código
                    window.inyectarCodigoDesdeCamara(decodedText);
                });
            },
            // Callback de escaneos fallidos (es normal que falle cuadro por cuadro, lo ignoramos)
            (errorMessage) => {
                // console.log(errorMessage); // Descomentar solo para debug
            }
        ).catch((err) => {
            // Callback de Error Crítico (Permisos denegados, sin cámara, etc.)
            $feedback.html('<i class="fas fa-exclamation-triangle"></i> No se pudo acceder a la cámara. Verifique los permisos del navegador.')
                    .attr('class', 'text-center mt-3 text-danger font-weight-bold');
            console.error("Error al iniciar Html5Qrcode:", err);
        });
    });

    // Evento: Cuando el modal de la cámara se cierra (sea por éxito o porque el usuario le dio a "Cancelar" / "X")
    $('#modalCamara').on('hidden.bs.modal', function () {
        // Si la cámara sigue activa al cerrar el modal, la matamos sin piedad (Previene pantalla negra en Android)
        if (html5QrCode && html5QrCode.isScanning) {
            html5QrCode.stop().then(() => {
                html5QrCode.clear();
            }).catch((err) => {
                console.error("Error al forzar detención de cámara:", err);
            });
        }
    });
});
